...
/* ---------- AUDIO LIFE-CYCLE DEBUGGING HELPER ---------- */
function logAudioState(contextMessage) {
    const bgMusic = document.getElementById('bgMusic');
    console.log(`[Audio Debug - ${contextMessage}]`, {
        audioExists: !!bgMusic,
        src: bgMusic ? bgMusic.src : null,
        paused: bgMusic ? bgMusic.paused : null,
        volume: bgMusic ? bgMusic.volume : null,
        muted: bgMusic ? bgMusic.muted : null,
        readyState: bgMusic ? bgMusic.readyState : null,
        error: bgMusic && bgMusic.error ? { code: bgMusic.error.code, message: bgMusic.error.message } : null,
...
function startBackgroundMusic() {
    const bgMusic = document.getElementById('bgMusic');
    console.log("[Audio Debug] startBackgroundMusic invoked. musicStarted state:", musicStarted);
    if (!bgMusic || musicStarted) return;
    musicStarted = true;

    initWebAudioAPI();
    if (audioCtx) {
        console.log(`[Audio Debug] Current AudioContext state before resume: ${audioCtx.state}`);
        if (audioCtx.state === 'suspended') {
            audioCtx.resume().then(() => {
                console.log("[Audio Debug] AudioContext resumed successfully in startBackgroundMusic.");
            }).catch(err => {
                console.error("[Audio Debug] AudioContext resume failed in startBackgroundMusic:", err);
            });
        }
    }

    // Fix the Web Audio Pre-Gain Volume Muting Bug:
    // If gainNode exists, we MUST keep the HTMLMediaElement volume at 1.0, 
    // otherwise the signal sent to the Web Audio nodes is completely silent!
    if (gainNode) {
        bgMusic.volume = 1.0;
    ...
    console.log("[Audio Debug] Attempting to play background music...");
    bgMusic.play().then(() => {
        console.log("[Audio Debug] bgMusic.play() resolved successfully. Starting volume fade-in...");
        ...
        const fadeIn = setInterval(() => {
            ...
            if (gainNode) {
                ...
                bgMusic.volume = 1.0; // Ensure it remains unmuted
            } else {
                bgMusic.volume = vol;
            }
            if (vol >= 0.4) {
                clearInterval(fadeIn);
                console.log("[Audio Debug] Fade-in complete. Final volume levels:", {
                    elementVolume: bgMusic.volume,
                    gainNodeValue: gainNode ? gainNode.gain.value : null
                });
            }
        }, 50);
    }).catch(error => {
        console.error("[Audio Debug] Audio playback failed or was blocked:", error);
    });
}
...
const CHAPTER_TITLES = [
    'The Day the World Smiled ✨', 'The Wonder Called You 🌸', 'The Story Time Wrote 📖',
    'The Hands Behind Your Smile 🤍', 'A Star That Still Shines ⭐', 'The Little Things I Treasure 🌹',
    'Where Our Paths Met ✨', 'The Words I Couldn\'t Keep 💌', 'The Next Chapter Awaits 🌙'
];
...
/* ---------- CHAPTER TRANSITION ---------- */
const TRANS_QUOTES = [
    'Every beautiful story deserves a beautiful beginning. Let\'s celebrate the day the world became a little brighter.',
    'Some people are remembered for their faces. Others are remembered for the kindness they leave behind.',
    'Every dream, every lesson, every smile... together they became the incredible person you are today.',
    'Behind every beautiful soul is a family that quietly filled it with love, values, and strength.',
    'Some people may no longer walk beside us, but their love continues to light every step we take.',
    'Sometimes admiration isn\'t found in grand moments—it\'s hidden inside the little things that make someone unforgettable.',
    'Among countless conversations and countless moments, some quietly become the memories we never want to lose.',
    'Some feelings stay silent for a long time... until one day, the heart decides they deserve to be heard.',
    'Every beautiful story leaves one page unfinished... hoping two hearts will write the rest together.'
];

const TRANS_MESSAGES = {
    "0-1": "Let the journey begin... ✨",
    "1-2": "Turning another beautiful page... 🌸",
    "2-3": "Let's walk through your story... ✨",
    "3-4": "Every journey begins at home... 🏡",
    "4-5": "Some stars never stop shining... ⭐",
    "5-6": "Here's what makes you unforgettable... 🌹",
    "6-7": "Now, our paths begin to cross... 💖",
    "7-8": "There are still a few words left... 💌",
    "8-9": "The story isn't over yet... 🌙"
};

function playChapterTransition(sourcePage, targetPage, callback) {
    ...
    // Set Loading Title Message
    const transitionKey = `${sourcePage}-${targetPage}`;
    if (titleText) {
        titleText.textContent = TRANS_MESSAGES[transitionKey] || "Turning another beautiful page... 🌸";
    }

    // Set Chapter Quote
    if (targetPage >= 1 && targetPage <= 9) {
        qt.textContent = '\u201c' + TRANS_QUOTES[targetPage - 1] + '\u201d';
    } else {
        qt.textContent = '\u201c' + TRANS_QUOTES[0] + '\u201d';
    }
...
function initWebAudioAPI() {
    ...
    console.log("[Audio Debug] initWebAudioAPI called. Current state:", {
        bgMusic: !!bgMusic,
        audioCtxExists: !!audioCtx,
        audioCtxState: audioCtx ? audioCtx.state : null
    });
    ...
    try {
        ...
        audioCtx.onstatechange = () => {
            console.log(`[Audio Debug] AudioContext state change detected: ${audioCtx.state}`);
        };
        ...
        console.log("[Audio Debug] Web Audio API initialized successfully. Context state:", audioCtx.state);
    } catch (e) {
        console.error("[Audio Debug] Web Audio API initialization failed:", e);
    }
}

document.addEventListener("DOMContentLoaded", () => {
    // Set up audio debugging events
    const bgMusic = document.getElementById("bgMusic");
    if (bgMusic) {
        console.log("[Audio Debug] Subscribing to audio element events.");
        const events = ['play', 'playing', 'pause', 'error', 'waiting', 'stalled', 'canplay', 'loadstart', 'loadedmetadata'];
        events.forEach(evt => {
            bgMusic.addEventListener(evt, (e) => {
                logAudioState(`AudioEvent: ${evt}`);
            });
        });
    }

    // Body click fallback to resume AudioContext
    document.body.addEventListener('click', () => {
        if (audioCtx && audioCtx.state === 'suspended') {
            console.log("[Audio Debug] Attempting to resume suspended AudioContext on body click...");
            audioCtx.resume().then(() => {
                console.log("[Audio Debug] AudioContext resumed on body click. State:", audioCtx.state);
            }).catch(err => {
                console.error("[Audio Debug] Failed to resume AudioContext on body click:", err);
            });
        }
    });
...
function initSecureGateway() {
    ...
    function verifyPasscode() {
        ...
        if (enteredInput === TARGET_PASS) {
            ...
            // Prime background music under user click gesture
            const bgMusic = document.getElementById("bgMusic");
            if (bgMusic) {
                initWebAudioAPI();
                if (audioCtx && audioCtx.state === 'suspended') {
...
function initNavigationEngine() {
    ...
    let ch2SubIndex = 0;
    const ch2SubKeys = ['radiance', 'essence'];
    ...
    function changeMainPage(targetPage, flowDirection = 'neutral') {
        ...
        playChapterTransition(currentMainPage, targetPage, () => {
            ...
            if (currentDOM) {
                ...
                currentDOM.style.display = 'none'; // Instantly hide under the black overlay to prevent vertical stacking and overlaps
            ...
            console.log(`[Audio Debug] Page traversal. TargetPage: ${currentMainPage}, TargetVolume: ${targetVol}`);
            if (gainNode && audioCtx) {
                bgMusic.volume = 1.0; // Ensure pre-gain is unmuted
...
function initFloatingAudioEngine() {
    ...
    toggleBtn.addEventListener("click", (e) => {
        e.stopPropagation();
        console.log("[Audio Debug] Manual toggle button clicked.");
        initWebAudioAPI();
        if (audioCtx && audioCtx.state === 'suspended') {
            audioCtx.resume().then(() => {
                console.log("[Audio Debug] AudioContext resumed via manual toggle.");
            });
        }
        if (bgMusic.paused) {
            if (gainNode) {
            ...
            console.log("[Audio Debug] Manual play requested.");
            bgMusic.play().then(() => {
                console.log("[Audio Debug] Manual play resolved.");
                syncPlaybackUI();
            }).catch(err => {
                console.error("[Audio Debug] Manual play failed:", err);
            });
        } else {
            console.log("[Audio Debug] Manual pause requested.");
            bgMusic.pause();
            syncPlaybackUI();
        }
    });
    ...
    // Call initially to sync the UI state with the paused music
    syncPlaybackUI();

    volumeSlider.addEventListener("input", (e) => {
        const val = parseFloat(e.target.value);
        console.log(`[Audio Debug] Volume slider input: ${val}. gainNode active: ${!!gainNode}`);
        if (gainNode) {
    ...
    if (cakeWrapper && micPermissionBtn) {
        ...
        const triggerBlowOutSequence = () => {
            ...
            setTimeout(() => {
                ...
                // Hide it fully after fade, then play the cinematic chapter transition overlay!
                setTimeout(() => {
                    playChapterTransition(0, 1, () => {
                        // Start the background music slowly after the transition is shown
                        startBackgroundMusic();

                        // Show the music pod
                        const musicPod = document.getElementById("musicPod");
                        if (musicPod) {
                            musicPod.classList.remove("hidden");
                        }

                        // --- THIS CODE EXECUTES WHEN THE SCREEN IS FULLY COVERED BY THE TRANSITION OVERLAY ---

                        // 1. Hide the cake completely
                        if (cakeContainer) {
                            cakeContainer.classList.add("hidden");
                            cakeContainer.style.display = "none";
                        }

                        // 2. Instantly set up global navigation elements under the black screen
                        const navElements = [
                        ...
                        // Make the chapter dots instantly ready
                        const chapterNav = document.getElementById("chapterNav");
                        if (chapterNav) {
                            const hops = chapterNav.querySelectorAll(".nav-hop");
                            hops.forEach(hop => hop.classList.add("revealed"));
                        }

                        // 3. Stagger the Chapter 1 content slide-up to match the exact moment the overlay fades away
                        const contentElements = [
                            { el: document.querySelector(".letter-body"), slideUp: true },
                            { el: document.getElementById("wishReveal"), slideUp: true }
                        ];
                        ...
                        contentElements.forEach((item, index) => {
                            ...
                            if (el) {
                                el.style.opacity = "0";
                                if (item.slideUp) el.style.transform = "translateY(25px)";
                                el.style.transition = "none";
                                ...
                                // Delay the slide-up slightly so it synchronizes with the black overlay fading out
                                setTimeout(() => {
                                    if (item.slideUp) {
                                        el.style.transition = "opacity 1.5s ease-out, transform 1.5s cubic-bezier(0.23, 1, 0.32, 1)";
                ...
                }, 1500); // Wait 1.5s for the cake to fully fade out before triggering the cinematic overlay
        ...
        micPermissionBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                // 1.5 second grace period before detecting sound to prevent accidental triggers from the mouse click
                setTimeout(() => {
                ...
                const checkBlow = () => {
                    ...
                    // Threshold to detect blowing (lowered to trigger with minimal sound volume/gentle blow)
                    if (average > 40) {
                        blowFrames++;
                    } else {
                    ...
                    // Require sustained blow for roughly 0.15s (10 frames)
                    if (blowFrames > 10) {
                ...
                if (pIndex < paragraphTexts.length) {
                    ...
                    if (proposalBoxContainer) {
                        ...
                        setTimeout(() => {
                            ...
                            const openCard = proposalBoxContainer.querySelector(".proposal-mail-card");
                            setTimeout(() => {
                                if (openCard) openCard.classList.add("revealed");
                            }, 200); // Allow brief delay for fade-in before sliding up
                        }, 50);
                    }
                    if (responseCard) {
